...
const char* current_job_sync = "current_job.txt";

const size_t squareSize = 10;
...
struct State {
    uint64_t seqno = 0;
    ComputationTask::TaskData taskData;

    State() = default;

    void ReadState(const char* statePath) {
        taskData.size = squareSize;
        std::ifstream in(statePath, std::ifstream::in);
        if (in.fail()) {
            seqno = 0;
            for (int i = 0; i < 7; ++i) {
                taskData.positions[i] = 0;
            }
            return;
        }
        in >> seqno;
        for (int i = 0; i < 7; ++i) {
            in >> taskData.positions[i];
        }
        in.close();
    }

    int NextState(const char* path) {
        FILE* f = fopen(path, "w");
        if (!f) return ERR_FOPEN;
        fprintf(f, "%lu %lu %lu %lu %lu %lu %lu %lu", squareSize, taskData.positions[0], taskData.positions[1], taskData.positions[2], taskData.positions[3], taskData.positions[4], taskData.positions[5], taskData.positions[6]);
        fclose(f);

        ++seqno;

        for (int i = 6; i >= 0; --i) {
            ++taskData.positions[i];
            auto possible = ComputationTask::PossibleToInitialize(taskData);
            if (!possible) {
                taskData.positions[i] = 0;
            } else {
                break;
    ...
    void WriteState(const char* statePath) {
        std::ofstream out(statePath, std::ofstream::out);
        out << seqno << " ";
        for (int i = 0; i < 7; ++i) {
            out << taskData.positions[i] << " ";
        }
        out.close();
    }
};
...
int make_job() {
    ...
    //
    sprintf(name, "%s_%d_%lu", app_name, start_time, state.seqno);

    retval = config.download_path(name, path);
    if (retval) return retval;
    retval = state.NextState(path);
    state.WriteState(config.project_path(current_job_sync));
...
int main(int argc, char** argv) {
    ...
    start_time = time(0);
    state.ReadState(config.project_path(current_job_sync));
